...
def chartsVaryICPC(data):
	...
	plt.plot(x1,r1,'ro',x2,r2,'ro',x15,r15,'ro')
	plt.savefig('runTimeICPC.png')
	plt.clf()
	plt.plot(x1,s1,'ro',x2,s2,'ro',x15,s15,'ro')
	plt.savefig('sitesICPC.png')
	plt.clf()
	plt.plot(x1,e1,'ro',x2,e2,'ro',x15,e15,'ro')
...
def generateArrays(data,x):
	...
	for d in data:
		...
		e_arr.append(d.entropy)
	return(x_arr,r_arr,s_arr,e_arr)


def generateDataSets(entropy,running,sites):
	data_sets = []
